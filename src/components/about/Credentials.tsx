  ...
  return (
    <div className="about-credentials my-10 rounded-xl border border-md-outline-variant p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h3 className="text-[0.8125rem] leading-5 uppercase tracking-widest text-md-on-surface-variant">
          Credentials
        </h3>
        <a
          href={CV_URL}
        ...
        {CERTIFICATIONS.map((c) => (
          ...
          >
            <div className="min-w-0">
              <span className="text-[0.9375rem] leading-6 leading-snug text-md-on-surface">
                {c.url ? (
                  <a
                    href={c.url}
                    target="_blank"
              ...
              {c.credentialId && (
                <p className="mt-0.5 font-mono text-[0.8125rem] leading-4 text-md-on-surface-variant">
                  {c.credentialId}
                </p>
              )}
            </div>
            <span className="shrink-0 text-[0.8125rem] leading-5 text-md-on-surface-variant">
              {c.issuer}
              {c.date ? ` · ${c.date}` : ""}
            </span>
          </li>
